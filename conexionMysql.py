...
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "djangoproyecto1.settings")
...
def insertaScanRecipe(var1,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11):
    db = mysql.connector.connect(user='root', password='lsuy0', host=settings.GLOBAL_VARIABLE_IP, database='orion', auth_plugin='mysql_native_password')
    cursor = db.cursor()
...
def insertaGraficaVulnerabilidades(var1,var2):
    db = mysql.connector.connect(user='root', password='lsuy0', host=settings.GLOBAL_VARIABLE_IP, database='orion', auth_plugin='mysql_native_password')
    cursor = db.cursor()
    sql = "INSERT INTO aplicacionweb1_graficavulnerabilidades (nombre_riesgo_vuln,cant_vuln_riesgo) VALUES (%s, %s)"
    values = ( var1, var2)
    cursor.execute(sql, values)
    db.commit()
    print(cursor.rowcount, "record inserted.")
